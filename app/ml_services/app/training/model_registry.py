...
def registry_relative_path(path: Path, models_root: Path) -> str:
    """Store registry paths with POSIX separators on every operating system."""
    ...
    try:
        ...
        raise ValueError(
            f"artifact is outside models root: {resolved}"
        ) from error
    return relative.as_posix()


def normalize_registry_path_value(value: str | Path) -> str:
    """Normalize old Windows registry paths without making them absolute."""
...
class ModelRegistry:
        ...
        registry.update(
            {
                ...
                "previous_model_version": None,
                "active_model_path": registry_relative_path(
                    target / "model.cbm",
                    self.root,
                ),
                "active_config_path": registry_relative_path(
                    target / "config.json",
                    self.root,
                ),
        ...
        history.append(
            {
                ...
                "decision": decision,
                "reasons": reasons or [],
                "roc_auc": metrics.get("classification", {}).get(
                    "roc_auc"
                ),
                "pr_auc": metrics.get("classification", {}).get(
                    "pr_auc"
                ),
                "total_net_return": metrics.get("trading", {}).get(
                    "total_net_return"
                ),
                "maximum_drawdown": metrics.get("trading", {}).get(
                    "maximum_drawdown"
                ),
            }
        )
    ...
    def promotion_gate(
        ...
        if (
            ...
            or pr_auc <= positive_rate
        ):
            reasons.append("PR AUC does not beat random baseline")

        if (
            int(probabilities.get("unique_count") or 0)
            < self.config.minimum_unique_probabilities
        ):
            reasons.append("probabilities have too few unique values")

        if float(probabilities.get("std") or 0.0) <= 1e-6:
            reasons.append("probabilities are effectively constant")

        if (
            int(trading.get("trades") or 0)
            < self.config.minimum_selected_trades
        ):
            reasons.append("too few selected trades")

        if (trading.get("mean_net_return") or 0.0) <= 0:
            reasons.append("mean net return is not positive")

        if (trading.get("total_net_return") or 0.0) <= 0:
            reasons.append("total net return is not positive")

        maximum_drawdown = trading.get("maximum_drawdown")
        if (
            maximum_drawdown is None
            or maximum_drawdown < self.config.maximum_allowed_drawdown
        ):
        ...
        if by_strategy:
            ...
            if strategy_share > 0.85:
                reasons.append("result is concentrated in one strategy")

        if champion_metrics:
            champion_trading = champion_metrics.get("trading", {})
            champion_classification = champion_metrics.get(
                "classification",
                {},
            )
            champion_total = champion_trading.get("total_net_return")
            candidate_total = trading.get("total_net_return")
            if champion_total is not None and candidate_total is not None:
                tolerance = max(abs(champion_total) * 0.05, 1e-6)
                if candidate_total < champion_total - tolerance:
                    reasons.append(
                        "candidate trading return is worse than champion"
                    )

            champion_dd = champion_trading.get("maximum_drawdown")
            if champion_dd is not None and maximum_drawdown is not None:
                if maximum_drawdown < champion_dd - 0.05:
                    reasons.append(
                        "candidate drawdown is materially worse than champion"
                    )
            ...
            if champion_brier is not None and candidate_brier is not None:
                if candidate_brier > champion_brier * 1.10:
                    reasons.append(
                        "candidate calibration is materially worse"
                    )
    ...
    def promote(self, version: str) -> dict[str, Any]:
        ...
        for required in ("model.cbm", "config.json", "metrics.json"):
            if not (candidate / required).exists():
                raise FileNotFoundError(
                    f"candidate artifact missing: {required}"
                )

        registry = self.read()
        previous_version = registry.get("active_model_version")
        previous_path = (
            self.active_dir / previous_version
            if previous_version
            else None
        )
        ...
        new_registry.update(
            {
                ...
                "active_model_version": version,
                "active_model_path": registry_relative_path(
                    final_active / "model.cbm",
                    self.root,
                ),
                "active_config_path": registry_relative_path(
                    final_active / "config.json",
                    self.root,
                ),
                "promoted_at": utc_now_iso(),
        ...
        if (
            ...
            and previous_path != final_active
        ):
        ...
        registry.update(
            {
                ...
                "active_model_path": registry_relative_path(
                    rollback_active / "model.cbm",
                    self.root,
                ),
                "active_config_path": registry_relative_path(
                    rollback_active / "config.json",
                    self.root,
                ),
                "promoted_at": utc_now_iso(),
